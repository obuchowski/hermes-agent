...
class TestResumePendingSystemNote:
    ...
    def test_empty_resume_turn_never_reaches_model_blank(self):
        ...
        assert "[System note:" in result
        assert "continue the latest unfinished user-authorized task" in result
    ...
    def test_resume_pending_empty_message_continues_unfinished_work(self):
        """An empty-message auto-resume startup turn must continue the latest
        unfinished user-authorized task from the next safe step. It may ask
        what to do only when the history contains no clear unfinished task.
        """
        entry = self._pending_entry(reason="restart_timeout")
        result = _simulate_note_injection(
            history=[
        ...
        assert "gateway restart" in result
        assert "continue the latest unfinished user-authorized task" in result
        assert "next safe step" in result
        assert "ask what they would like to do next" not in result
        assert "skip any unfinished work" not in result
