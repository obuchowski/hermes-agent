        ...
        if agent._session_db:
            try:
                ...
                if in_place:
                    ...
                    # Snapshot continuation metadata before ending the parent.
                    # A failed/missing read is not permission to mint a child
                    # with a different workspace/profile identity.
                    _old_session_row = agent._session_db.get_session(agent.session_id)
                    if not isinstance(_old_session_row, dict):
                        raise RuntimeError(
                            f"Cannot rotate missing parent session {agent.session_id}"
                        )
                    old_title = _old_session_row.get("title")
                    old_cwd = _old_session_row.get("cwd")
                    old_profile_name = _old_session_row.get("profile_name")
